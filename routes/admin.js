...
const adminAuth = async (req, res, next) => {
    ...
    if (!authHeader || !authHeader.startsWith('Basic ')) {
        return res.status(401).json({ message: 'Unauthorized: Admin access required (Missing Basic Auth)' });
    }
    ...
    try {
        ...
        if (!passwordMatch) {
             console.warn(`Authentication failed: Incorrect password for user '${username}'.`);
            return res.status(401).json({ message: 'Unauthorized: Invalid credentials' });
        }
...
router.post('/tracking', adminAuth, async (req, res) => {
    ...
    try {
        ...
        if (!recipientName || !recipientPhone || !destination || !origin || !status || !service) {
...
router.put('/tracking/:trackingNumber', adminAuth, async (req, res) => {
    ...
    try {
        ...
        if (updates.estimatedDeliveryDate) updates.estimatedDeliveryDate = new Date(updates.estimatedDeliveryDate);
...
router.delete('/tracking/:trackingNumber', adminAuth, async (req, res) => {
    try {
        const trackingNumber = req.params.trackingNumber;
        const result = await db.delete(schema.trackings)
            .where(eq(schema.trackings.trackingNumber, trackingNumber))
